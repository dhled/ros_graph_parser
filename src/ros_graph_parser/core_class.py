    ...
    def types_struc(self,struc_dict,indent):
        ...
        for struc_element in struc_dict:
            ...
            if isinstance(sub_value,dict):
                str_param += self.types_struc(struc_dict[struc_element],indent_new)
            str_param += ",\n"
        str_param = str_param[:-2]
        str_param+="}"
        indent_new=""
    ...
    def value_struc(self,struc_dict,indent,string_end):
        ...
        for struc_element in struc_dict:
            ...
            if sub_type =="String":
                sub_value="'"+sub_value+"'"
            if sub_type == 'List':
                sub_value=str(sub_value).replace("[","{").replace("]","}").replace("{{","{").replace("}}","}")
            if sub_type == "Boolean":
...
class Node(object):
    ...
    def dump_java_ros_model(self):
        ...
        ros_model_str+=self.params.java_format_ros_model("        ", "Parameters", "parameter")
        ros_model_str+="}},\n"
    ...
    def dump_java_system_model(self, package=""):
        ...
        system_model_str+=self.action_servers.java_format_system_model("            ", "ActionServers", "Server", self.name, package, "ActionServer")
        system_model_str+=self.action_clients.java_format_system_model("            ", "ActionClients", "Client", self.name, package, "ActionClient")
        system_model_str+=self.params.java_format_system_model("            ", "Parameters", "Parameter", self.name, package)
        system_model_str+="},\n"
